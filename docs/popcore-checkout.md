...
Customer pays is prominent and editable. Cash, E-transfer, WeChat Pay and Alipay use the existing whole-dollar suggestion near a 2.5% reduction, including the approved $20 to $19 exception. Card alone uses the exact original total. A split that includes Card gets no cash discount: totals of at least $10 are floored to the whole dollar before the first payment, while smaller totals remain exact. For example, $7.90 stays $7.90, $28.31 becomes $28.00 and $31.91 becomes $31.00. The cashier marks “This split includes Card” before collecting either tender, so either method may be paid first. Discounts above 20% of the original tax-inclusive total produce a warning, without requiring manager approval.

The pre-tax Clover discount or rounding adjustment is explicitly an estimate from the original subtotal and tax. Differing targets wait for a confirmed Clover total and cannot overwrite the captured payment total.
...
The September 22 tender-pricing update added focused integer-cent checks for Card-only, discount-eligible and Card-inclusive split targets, plus a browser contract for selecting the Card-inclusive split before payment. This remains local verification, not live Clover or production evidence.
