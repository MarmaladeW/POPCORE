...
export default function ScheduleDayPanel({ date, storeCode, employees, availability, shifts, onAssign, onEdit }: Props) {
      ...
      <section><strong>Assigned · {assigned.length}</strong>
        ...
        {assigned.map(shift => {
          ...
          return <div className="pc-schedule-person" key={shift.id}><div>
            <strong>{shift.employee_name || employees.find(employee => employee.id === shift.employee_id)?.name || 'Employee'}</strong>
