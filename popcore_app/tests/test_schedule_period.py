...
class SchedulePeriodTests(IsolatedApiCase):
    def payload(self, **changes):
        data = {'period_start': '2026-09-21', 'store_code': 'DT', 'version': 0,
                'days': [{'date': str(dt.date(2026, 9, 21) + dt.timedelta(days=i)),
    ...
    def get(self, role='staff', store='DT'):
        return self.client.get('/api/schedule/availability/period', query_string={'period_start': '2026-09-21', 'store_code': store}, headers=self.headers(role))
    ...
    def test_submission_resubmission_scope_and_metadata(self):
        ...
        self.assertEqual(result.json['period_end'], '2026-10-04')
    ...
    def test_shift_assignment_guards_preserve_shifts(self):
        ...
        shift = {'employee_id': employee, 'store_code': 'DT', 'date': '2026-09-21', 'start_time': '10:00', 'end_time': '17:00', 'require_availability': True}
        def create(data):
        ...
        self.assertEqual(create(shift | {'date': '2026-09-26'}).status_code, 409)
        result = create(shift)
    ...
    def test_migration_preserves_legacy_ids_and_is_repeatable(self):
        ...
        with closing(self.connect()) as con:
            ...
            con.execute("INSERT INTO availability VALUES (42,?,'2026-09-21','09:00','17:00','old','created','updated',?)", (employee,self.store_id))
    ...
    def test_malformed_write_payloads_return_validation_errors(self):
        employee = self.client.get('/api/schedule/me', headers=self.headers()).json['id']
        shift = {'employee_id': employee, 'store_code': 'DT', 'date': '2026-09-21', 'start_time': '10:00', 'end_time': '17:00'}
    ...
    def test_fortnight_notes_and_checklist_have_separate_keys(self):
        ...
        for resource, fields in [('notes', {'content': 'Two weeks'}), ('checklist', {'employee_id': employee, 'considered': True})]:
            ...
            result = self.client.put(path, json=fields | {'period_key': 'fortnight:2026-09-21'}, headers=self.headers('manager'))
            self.assertEqual(result.status_code, 200, result.json)
            result = self.client.get(path, query_string={'key': 'fortnight:2026-09-21'}, headers=self.headers('manager'))
            self.assertEqual(result.status_code, 200)
            weekly = self.client.get(path, query_string={'key': 'week:2026-09-21'}, headers=self.headers('manager')).json
    ...
    def test_availability_lists_keep_rows_and_submission_in_one_snapshot(self):
        ...
        for path, role in [('/api/schedule/availability/me', 'staff'),
            ...
            with self.subTest(path=path):
                ...
                row = next(day for day in response.json if day['date'] == '2026-09-21')
